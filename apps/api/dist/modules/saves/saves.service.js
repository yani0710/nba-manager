...
class SavesService {
    ...
    async finalizeMatchSimulation(saveId, gameId, payload) {
        ...
        const loadForTeam = (teamId) => (Number(teamId) === Number(game.homeTeamId) ? homeLoad : awayLoad);
        ...
        this.updateTeamStateAfterGame(currentData, game.homeTeamId, game.awayTeamId, homeScore, awayScore);
    ...
    async getDashboardSummary(id) {
        ...
        const coachResponseImpact = responseDeltas.length > 0
            ...
            : 0;
        const moraleScore = this.clamp(Math.round(avgRosterMorale * 0.55
            + (recentWinRate * 100) * 0.2
            + trainingRating * 0.15
            + (coachResponseImpact * 8)
            - (avgRosterFatigue * 0.1)), 0, 100);
        const chemistryScore = this.clamp(Math.round((moraleScore * 0.55) + (trainingRating * 0.25) + (teamForm * 0.2)), 0, 100);
