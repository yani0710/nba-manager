...
function simulateGame(homePlayers, awayPlayers, homeTeamRating, awayTeamRating, options = {}) {
...
function getTeamRating(players, teamForm = 50) {
    if (players.length === 0)
        return 70;
...
function getDefensePenalty(scheme) {
    if (scheme === "switch")
        return 0.02;
    if (scheme === "press")
        return 0.01;
...
function buildStats(players, teamScore, paceFactor) {
    if (players.length === 0)
        return [];
    ...
    const rawWeights = players.map((player, idx) => {
        ...
        if (idx < 5)
            return (1.0 - idx * 0.08) * positionBoost * talentBoost * formBoost;
...
function getPositionUsageBoost(position) {
    const p = String(position ?? "").toUpperCase();
    if (p.includes("PG") || p.includes("SG") || p === "G")
        return 1.1;
    if (p.includes("C"))
        return 0.95;
    return 1.0;
}
function getReboundBias(position) {
    const p = String(position ?? "").toUpperCase();
    if (p.includes("C") || p.includes("PF"))
        return 1.5;
    if (p.includes("PG"))
        return -0.5;
    return 0;
}
function getAssistBias(position) {
    const p = String(position ?? "").toUpperCase();
    if (p.includes("PG"))
        return 1.8;
    if (p.includes("C"))
...
function getThreePointAttemptRate(position) {
    const p = String(position ?? "").toUpperCase();
    if (p.includes("PG") || p.includes("SG"))
        return 0.42;
    if (p.includes("SF"))
        return 0.34;
    if (p.includes("PF"))
...
function normalizeTeamPoints(stats, targetPoints) {
    if (!stats.length)
        return;
    ...
    while (delta !== 0 && guard < 2000) {
        ...
        if (delta < 0 && ordered.every((s) => s.points <= 0))
            break;
    }
    // Recompute performance rating with updated points (other fields unchanged).
    for (const s of stats) {
        const missedShotsPenalty = Math.max(0, (s.twoPtAtt - s.twoPtMade) + (s.threePtAtt - s.threePtMade) + (s.ftAtt - s.ftMade));
