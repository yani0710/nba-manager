...
function asUnitPercent(value: number): number {
  return value > 1 ? value / 100 : value;
}

function toPosGroup(position: string): PositionGroup {
...
function weightedOverall(pos: PositionGroup, att: number, play: number, def: number, phy: number): number {
  if (pos === "PG") return att * 0.35 + play * 0.3 + def * 0.2 + phy * 0.15;
  if (pos === "SG") return att * 0.4 + play * 0.2 + def * 0.2 + phy * 0.2;
  if (pos === "SF") return att * 0.3 + play * 0.15 + def * 0.25 + phy * 0.3;
  if (pos === "PF") return att * 0.25 + play * 0.1 + def * 0.3 + phy * 0.35;
  return att * 0.2 + play * 0.1 + def * 0.35 + phy * 0.35;
}

function metricRaw(input: ImpactRatingInput) {
...
export function computeImpactRatings(inputs: ImpactRatingInput[]): ImpactRatingOutput[] {
  const byGroup = new Map<PositionGroup, ImpactRatingInput[]>();
  ...
  const output: ImpactRatingOutput[] = [];
...
export function computeTeamForm(players: Array<{ overall: number; dpm: number }>): number {
