...
function buildGameweekRanges(season: string): GameweekRange[] {
  const startYear = Number(String(season ?? "2025-26").slice(0, 4)) || 2025;
  const out: GameweekRange[] = [];
...
export function getGameweekForDate(season: string, date: Date | string): number {
  ...
  if (value < ranges[0].start) return 1;
  return 24;
}

export function getGameweekRanges(season: string): Array<{ week: number; start: string; end: string }> {
