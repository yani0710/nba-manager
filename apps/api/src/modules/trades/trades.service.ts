...
export class TradesService {
  ...
  async respondTradeProposal(dto: RespondTradeProposalDto) {
    const save = await this.ensureSave(dto.saveId);
    const proposal = await prisma.tradeProposal.findFirst({
    ...
    if (!["PENDING", "COUNTERED"].includes(proposal.status)) {
      throw new BadRequestError("Only pending/countered proposals can be responded to");
    }
  ...
  async generateTradeBlockInterestForDay(saveId: number, day: number, date: Date) {
    const save = await this.ensureSave(saveId);
    const payload = (save.data ?? {}) as {
      rosterManagement?: { tradeBlockPlayerIds?: number[] };
    };
    ...
    if (!managedTeamId || tradeBlockPlayerIds.length === 0) return 0;

    const activePending = await prisma.tradeProposal.count({
      where: {
        saveId,
    ...
    if (listedPlayers.length === 0) return 0;
    ...
    for (const listed of listedPlayers) {
      ...
      const duplicate = await prisma.tradeProposal.findFirst({
        where: {
          saveId,
          fromTeamId: buyer.id,
          toTeamId: managedTeamId,
      ...
      const buyerCandidates = await prisma.player.findMany({
        where: {
          teamId: buyer.id,
          active: true,
          id: { not: listed.id },
